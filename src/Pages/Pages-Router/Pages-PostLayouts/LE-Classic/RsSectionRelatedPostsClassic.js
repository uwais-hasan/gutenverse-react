import React from 'react';
import {NavLink} from "react-router-dom";
import CameraAltIcon from "@mui/icons-material/CameraAlt";

const RsSectionRelatedPostsClassic =()=>{
    return(
       <div className='section-RelatedPost-Rl-Parent'>
           <div className='section-post-relatedPost-RL'>
               <div className='section-post-relatedPost max-width-none-important'>
                   <h3 className='style-global-title '><span className='border-title'>Related</span> Posts</h3>
                   <div className="section_Latest_Stories">
                       <div className="content">
                           <div className="content-img">
                               <NavLink to='/main/A great thing made of gold. Down the hill I saw it.'>
                                   <img src="../img-news/Link-img/link-5.jpg"/>
                               </NavLink>
                           </div>
                           <div className="content-info">
                               <NavLink to='/main/A great thing made of gold. Down the hill I saw it.'>  <h2 className="style-title">A great thing made of gold. Down the hill I saw it.</h2></NavLink>
                               <span className=" style-global-span-info letter-space-2">FEBRUARY 14, 2019</span>
                               <span className='padding-0-10'>.</span>
                               <NavLink to='/category/Featured' className=" style-global-span-info letter-space-3  ">FEATURED</NavLink> ,
                               <NavLink to='/category/Lifestyle' className=" style-global-span-info letter-space-3  ">LIFESTYLE</NavLink> ,
                               <NavLink to='/category/Photo' className=" style-global-span-info letter-space-3 "> PHOTO </NavLink>
                               <p className="style-global-text-info-3">
                                   I shouted above the sudden noise. She looked away from me downhill. The people were coming out …
                               </p>
                           </div>
                           <CameraAltIcon className='absolute-content'/>

                       </div>
                       <div className="content">
                           <div className="content-img">
                               <NavLink to='/main/It must be in a month, but I see nothing here'>
                                   <img src="../img-news/Link-img/link-12.jpg"/></NavLink>
                           </div>
                           <div className="content-info">
                               <NavLink to='/main/It must be in a month, but I see nothing here'>  <h2 className="style-title">It must be in a month, but I see nothing here</h2></NavLink>
                               <span className=" style-global-span-info letter-space-2">DECEMBER 3, 2018</span>
                               <span className='padding-0-10'>.</span>
                               <NavLink to='/category/LifeStyle' className=" style-global-span-info letter-space-3  ">LIFESTYLE </NavLink> ,
                               <NavLink to='/category/Trending' className=" style-global-span-info letter-space-3 "> TRENDING </NavLink>
                               <p className="style-global-text-info-3">
                                   Through two long weeks I wandered,
                                   stumbling through the nights
                                   guided only by the stars and hi…
                               </p>
                           </div>

                       </div>
                       <div className="content">
                           <div className="content-img">
                               <NavLink to='/main/Audio Post: A sigh of satisfaction'>
                                   <img src="../img-news/Link-img/link-13.jpg"/></NavLink>
                           </div>
                           <div className="content-info">
                               <NavLink to='/main/Audio Post: A sigh of satisfaction'>  <h2 className="style-title">Audio Post: A sigh of satisfaction</h2></NavLink>
                               <span className=" style-global-span-info letter-space-2">SEPTEMBER 10, 2018</span>
                               <span className='padding-0-10'>.</span>
                               <NavLink to='/category/Photo' className=" style-global-span-info letter-space-3  "> PHOTO </NavLink> ,
                               <NavLink to='/category/Trending' className=" style-global-span-info letter-space-3  ">TRENDING </NavLink>

                               <p className="style-global-text-info-3">
                                   When the amphitheater had cleared I crept stealthily to the top and as the great excavation lay…</p>
                           </div>
                       </div>
                   </div>
               </div>
           </div>
       </div>
    )
}

export default RsSectionRelatedPostsClassic;